...
function pageChange() {
	HouseTab.value = houses.value.slice(
		currentPage.value * currentPageSize.value - currentPageSize.value,
...
function getHouses() {
	// 获取房源总列表
	axios
		.get("https://mock.mengxuegu.com/mock/69a4fd728c3de17d818e5d49/example/house")
		.then((res) => {
			...
			houses.value.forEach((item) => {
				var flag = true;
				location.value.forEach(function (it) {
					// @ts-ignore
...
async function AddHouse() {
	await formEl.value.validate((valid) => {
		if (valid) {
			...
			axios
				.get("http://localhost:8082/AddHouse", { params: Addhouse.value })
				.then((res) => {
					if (res.data) {
						getHouses();
...
function deleteHouse(row) {
	ElMessageBox.confirm("确定删除该房源吗？", "提示", {
		...
		axios
			.get("http://localhost:8082/deleteVUE", { params: { id: row.id } })
			.then((res) => {
				if (res.data) {
...
const Houserules = ref({
	location: [{ required: true, message: "请输入房源地区", trigger: "blur" }],
...
<template>
	<el-row id="house-container" align="middle" justify="start">
		...
		<el-row
			justify="center"
			align="middle"
			class="borderClass house-search-container"
			v-if="SearchWindow"
		>
			...
			<el-col :span="6" style="text-align: right">
				...
				<el-button type="default"
					>重置<template #icon
						><el-icon><Delete /></el-icon></template
				></el-button>
			</el-col>
		</el-row>
		...
		<el-row
			...
			align="top"
			class="borderClass"
			style="margin-top: 20px; flex-grow: 1; padding: 25px; height: 200px"
		>
			<el-row style="width: 100%">
				<el-col :span="16">
					<el-button type="primary" @click="AddDrawer = true"
						>新增房源
						<template #icon
							><el-icon><CirclePlus /></el-icon
						></template>
					</el-button>
					<el-button type="primary" @click="exportHousesExcel" plain
						>导出房源数据
						<template #icon
							><el-icon><Download /></el-icon
						></template>
					</el-button>
					<el-button type="primary" plain @click="plAddHouseWindow = true"
						>批量导入房源
						<template #icon
				...
				>
					...
					<el-table-column type="index" label="序号" width="60" align="center">
						<template #default="scope">{{
							scope.$index + currentPageSize * (currentPage - 1) + 1
						}}</template>
					</el-table-column>
					<el-table-column
						label="房源地区"
					...
					<el-table-column label="操作" width="300" fixed="right">
						<template #default="scope">
							...
							<el-button type="danger" size="small" @click="deleteHouse(scope.row)"
								>删除
								<template #icon>
