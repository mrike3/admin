...
function refresh() {
	if (MStore.currentTab.isKeepAlive) {
		...
		MStore.KeepAliveNames = MStore.KeepAliveNames.filter(
...
function closeOthers() {
	MStore.tabList = MStore.tabList.filter(
		(item) => item.name == "/home" || item.name == MStore.activeTabName,
	);
	if (!MStore.currentTab.isKeepAlive) {
		MStore.KeepAliveNames = [];
	} else {
		MStore.KeepAliveNames = MStore.KeepAliveNames.filter(
			(item) => item == MStore.currentTab.KeepName,
		);
	}
}
//关闭所有标签
...
//关闭左侧标签
function closeLeft() {
	const tabs = MStore.tabList.splice(
		1,
		MStore.tabList.findIndex((item) => item.name === MStore.currentTab.name) - 1,
	);
	tabs.forEach((item) => {
		if (item.isKeepAlive) {
	...
	<el-dropdown trigger="click">
		...
		<template #dropdown>
			...
			<el-dropdown-menu>
				<el-dropdown-item
					icon="Remove"
					v-bind:disabled="MStore.activeTabName == '/home'"
					@click="closeCurrent"
				>
