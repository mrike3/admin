...
<template>
	...
	>
		...
		<el-row style="width: 100%; padding: 10px" v-if="HStore.SearchList.length > 0">
			<div
				class="search-item"
				v-for="(item, index) in HStore.SearchList"
				:key="index"
		...
		<el-empty
			...
			style="margin: 20px"
			:image-size="100"
			:description="$t('header.SearchNull')"
		/>
		<el-row class="Search-btn" align="middle">
			...
			<div class="btn-item">
				<el-icon class="btn-icon">
					<el-text style="font-size: 10px"> ESC </el-text>
				</el-icon>
				{{ $t("header.SearchBtn.esc") }}
			</div>
			<el-tag type="warning" style="position: absolute; right: 10px"> No Function </el-tag>
		</el-row>
	</el-dialog>
	...
	<el-dropdown style="margin-left: 10px" trigger="click" @command="HStore.toggleLanguage">
		...
		<template #dropdown>
			<el-dropdown-menu>
				<el-dropdown-item command="zh" v-bind:disabled="HStore.isZh"
					>简体中文</el-dropdown-item
				>
				<el-dropdown-item command="en" v-bind:disabled="!HStore.isZh"
	...
	<el-dropdown style="margin-left: 10px; margin-right: 10px" trigger="click">
		...
		<template #dropdown>
			<el-dropdown-menu>
				...
				<el-dropdown-item>
					<icon-github theme="outline" class="icon1" />&nbsp; Github
				</el-dropdown-item>
				<el-dropdown-item>
